 // JavaScript Document
var obj = [1000,1500,500,2000,1000,1000,1800];
...
function PieChart(data,name) {
	...
	/*this.mycan.style.backgroundColor = "turquoise";*/
	this.cwidth = this.mycan.width;
	this.cheight = this.mycan.height;
	this.ctx = this.mycan.getContext("2d");
	this.mycolor = ["red","pink","green","blue","yellow","purple","violet","black","grey","cyan","orange","black","maroon"];
	var that = this;
	...
	this.angleCalculate = function() {
		...
		for(var i = 0;i<that.dataArr.length;i++) {
		  that.sum += that.dataArr[i];	
		}
		
		for(var j = 0;j<that.dataArr.length;j++) {
		  value[j] = ((that.dataArr[j]/that.sum)*100).toFixed(2);
		  newObj[j] = (that.dataArr[j]/that.sum)*2*Math.PI;	
		}
    ...
    if (that.last < (Math.PI)) {
	 ...
	  that.ctx.stroke();
	  ...
	  var txty = that.cheight+ Math.sin(middle) * that.radius*1.4 - 12;
	   ...
	   that.ctx.fillText(value[k]+"%", txtx, txty + 22);
	   that.ctx.restore(); 
	   that.last += object[k]; 
    }
  }
